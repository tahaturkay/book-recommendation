...
function BookCard({ book, onCardClick }) { // book parametresi ile backendden gelen kitabı alıyor
  ...
  async function handleAddToLibrary (e) {
    e.stopPropagation(); // Kartın başka bir tıklama özelliğini (örn: detaya gitme) tetiklemesini engeller
    try{
      await addToLibraryRequest(book.bookID);
      console.log("abi senin kitabi favorilere ekledik");
    }catch(error){
      console.error(error);
    }
  };
  ...
  const handleRemoveFromLibrary = async (e) => {
      ...
      try {
        ...
        // Silme başarılıysa, Kütüphane sayfasına "Bu kitabı ekrandan kaldır" diye haber ver
        if (onRemove) {
  ...
  >
    ...
    <div style={{ position: 'relative', height: '65%', width: '100%' }}>
      <img 
        // kitabın kapak fotosu yoksa default foto bu
